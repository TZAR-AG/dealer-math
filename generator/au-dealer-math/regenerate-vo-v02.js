// Regenerate AU Dealer Math V2 voiceover via ElevenLabs API.
// Reads the V2 script, splits into scenes, renders each at locked Macca
// voice (Paul) settings, saves to v02-renders/voice/.
//
// Run from repo root: node generator/au-dealer-math/regenerate-vo-v02.js
//
...
const SCRIPT_PATH = path.resolve(
  __dirname, '..', '..',
  'content', 'au-dealer-math', 'scripts', 'v02-finance-managers-office.md'
);
const OUT_DIR = path.resolve(
  __dirname, '..', '..',
  'content', 'au-dealer-math', 'scripts', 'v02-renders', 'voice'
);

// Map V2 script scene headers to output filenames.
// findFilenameForTitle does case-insensitive substring match on the title,
// so each key just needs a unique prefix that appears in exactly one V2 header.
const SCENE_FILES = {
  'HOOK': 'vo-scene-1-hook.mp3',
  'AUTHORITY': 'vo-scene-2-authority.mp3',          // [0:15-1:00] AUTHORITY ANCHOR + THREE-ROOM REANCHOR
  'WHAT THE F&I': 'vo-scene-3-fnioffice.mp3',       // [1:00-2:30] WHAT THE F&I ROOM IS
  'RATE RANGE': 'vo-scene-4-rate-game.mp3',         // [2:30-4:30] THE RATE RANGE GAME
  'COMMISSION POOL': 'vo-scene-5-commission.mp3',   // [4:30-6:00] THE COMMISSION POOL...
  'WHAT TO ACTUALLY': 'vo-scene-6-fix.mp3',         // [6:00-7:30] WHAT TO ACTUALLY DO
  'V3 PREVIEW': 'vo-scene-7-signoff.mp3',           // [7:30-8:30] V3 PREVIEW + SIGN-OFF
};
