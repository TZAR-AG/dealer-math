"""Swap V1 master audio: Storyblocks (HAAWK Content ID flagged) -> YT Audio Library Underbelly trio.

Process:
1. Stitch 3 Underbelly & Ty Mayer tracks with 2s crossfades
2. Loop-extend if shorter than V1 video duration (587.18s)
...
5. Output: video/out/audm-v1-payment-not-price-pivot-v2.mp4

YT Content ID safe — YT Audio Library tracks can't conflict with YT's own DB.

Run: python generator/au-dealer-math/swap-v1-music.py
"""
import subprocess
...
VO = REPO / "content/au-dealer-math/scripts/v01-renders/voice/master-vo.mp3"
V1_VIDEO = REPO / "video/out/audm-v1-payment-not-price-pivot.mp4"
...
OUT_MUSIC = OUT_DIR / "_audm-v1-music-bed-v2.mp3"
OUT_AUDIO = OUT_DIR / "_audm-v1-audio-v2.mp3"
OUT_VIDEO = OUT_DIR / "audm-v1-payment-not-price-pivot-v2.mp4"
...
# Pre-flight checks
for f in [VO, V1_VIDEO, *TRACKS]:
    if not f.exists():
        print(f"[FAIL] Missing: {f}")
        exit(1)

target_duration = probe_duration(V1_VIDEO)
print(f"V1 video duration: {target_duration:.2f}s ({target_duration/60:.2f} min)")
for t in TRACKS:
    d = probe_duration(t)
